...
모듈화 설계의 핵심은 캡슐화다. 캡슐화가 잘 되어 있다면 무언가 변경해야 할 때 고려해야 할 모듈 수가 적어져 코드 변경하기가 훨씬 쉬워진다. 객체 지향을 처음 배울 땐 캡슐화가 필드를 숨기는 것이라 배우지만 역할은 훨씬 많다.
...
위임 객체를 캡슐화할 때는 클라이언트가 위임 객체의 또 다른 기능을 사용하고 싶을 때마다 서버에 위임 메서드를 추가하는 과정이 성가셔진다. 이런 겨웅 직접 호출하는 게 나을 수도 있다.
...
3. 대체할 알고리즘을 준비한다.
4. 정적 검사를 수행한다.
5. 기존 알고리즘과 새 알고리즘의 결과를 비교하는 테스트를 수행한다. 두 결과가 같다면 리팩터링이 끝나고 그렇지 않다면 기존 알고리즘을 참고해 새 알고리즘을 테스트하고 디버깅한다.
